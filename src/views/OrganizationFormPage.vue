...
export default {
    ...
    methods: {
        async createOrganization(newOrg) {
            ...
            try {
                const response = await axios.post('http://127.0.0.1:8000/organizations', newOrg);
        ...
        async updateOrganization(updatedOrg) {
            ...
            try {
                const response = await axios.put(`http://127.0.0.1:8000/organizations/${updatedOrg.id}`, updatedOrg);
        ...
        resetForm() {
            ...
            this.organization = { name: "", createdDate: "", disabled: false, id: null };
    ...
    created() {
        ...
        if (orgId) {
            ...
            const existingOrg = this.organizations.find(org => org.id === parseInt(orgId));
            if (existingOrg) {
